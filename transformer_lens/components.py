...
class AbstractAttention(ABC, nn.Module):
    def __init__(
        ...
        self.q_lns = nn.ModuleList([LayerNorm(cfg, length=cfg.d_head) for _ in range(cfg.n_heads)])
        self.k_lns = nn.ModuleList([LayerNorm(cfg, length=cfg.d_head) for _ in range(cfg.n_heads)])
